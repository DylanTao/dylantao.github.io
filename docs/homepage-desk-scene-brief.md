...
- Current interaction priority: the interior room view should be a full 360 center-room look-around. Dragging in room mode rotates the view from inside the room with bounded pitch; it must not move the camera into an exterior orbit unless the user deliberately clicks into outside mode.
...
- Interior drag is a full 360 center-room look-around: the camera stays inside the room, yaw can rotate all the way around, and exterior-looking-in composition only appears after the deliberate outside-mode window click.
