...
- Current interaction priority: the interior room view should be a full 360 center-room look-around. Dragging in room mode rotates the view from inside the room with bounded pitch; it must not move the camera into an exterior orbit unless the user deliberately clicks into outside mode. The full yaw range must preserve visible wall clearance, and the 180-degree rear view should show rear-room floor, trim, and decor at a believable distance rather than a camera-collided wall close-up.
...
- The current implementation centralizes room bounds plus the window, desk, onsen, chair, and rack anchors. Those anchors drive the interior objects and the exterior window/miniature projection; camera positions, look targets, axis compression, and small legibility offsets remain view-specific framing parameters. The exterior uses a few extruded irregular cliff faces plus lightweight terrace meshes; preserve the shared anchor contract before introducing a heavier asset pipeline.
