...
- Combined blog-post publish to main: public total moved to 290 commits, 2.55B tokens, 238 hours, 1530 kWh, and roughly 1.0 ten-year urban tree stored-carbon equivalent while API-cost and CodexBar labels stayed rounded at about $820 API cosplay and about $2.2K; the path-scoped desk counter stayed at 107 commits, while retained-session desk-window totals moved to 1.49B tokens, 124 hours, 894 kWh, and about $470 API cosplay.
- Evidence: `python bin/audit_agentic_usage.py --write --include-pending-commit` scanned `C:\Users\dylan\.codex\sessions\2026`, found 95 repo sessions, counted 68 sessions after the revamp cutoff and 33 after the desk cutoff, and refreshed `_data/agentic_usage.yml` for the pending main merge that publishes both July 5 blog posts.
