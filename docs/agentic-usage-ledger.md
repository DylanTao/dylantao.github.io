...
- Pending public totals: 350 revamp commits, 5.7B rounded retained-session tokens, 408 rounded agent-hours, 3420 kWh, about 2.1 ten-year urban trees' stored-carbon equivalent, and about $4.9K under the request-aware API-rate replay. The desk window remains 119 path-scoped commits; its time-window estimate moved to 4.61B rounded tokens and 294 rounded hours.
- Evidence: `python bin/audit_agentic_usage.py --write --include-pending-commit` scanned 634 local and 252 repo-attributed leaf sessions, retained 40,019 additive usage-event sources after skipping 218,646 copied/repeated snapshots and 89,269 fork-preamble events, and observed 767 retained post-cutover turns. The full Python suite passed 111 tests, including 35 focused ledger-audit tests, and the exact browser reproduction produced `windowJump` with no runtime errors after forcing the canvas above the viewport.
