...
For whole-account Codex activity, `_data/agentic_usage.yml.account_lifetime` is primary. It is a verified snapshot from authenticated Codex account activity at `GET /wham/profiles/me`, not an inference from local files. `python bin/refresh_codex_account.py --write` performs the bounded account-only refresh without invoking a model or scanning retained logs. It reads the current access token and account id from `~/.codex/auth.json` in memory, fails closed on authentication or contract errors, and emits only a sanitized status line. Use `--dry-run`, `--input FIXTURE.json`, and `--json-status` for validation and scheduler integration; never pass credentials as arguments.

The account-only collector validates cumulative parity, monotonic lifetime/task/peak totals, the source-local partial date, the complete latest 30-day calendar range, and the exact derived Sunday buckets before staging any output. The rolling range spans five or six calendar-week buckets depending on its endpoints. The account endpoint omits genuine zero-token dates, so the collector restores zeros only inside a source-bounded 30-day range. A semantic change publishes immediately; an unchanged snapshot is republished only after an 18-hour heartbeat threshold so the public source timestamp remains under 24 hours old. Exit codes are stable for unattended recovery: `20` authentication, `21` endpoint/network, `22` schema/contract, and `23` stale or non-monotonic input.
...
The only current numeric snapshot is `_data/agentic_usage.yml`. `bin/audit_agentic_usage.py` regenerates the repo and retained-local scopes and refreshes their price replays; the verified account-activity response is stored separately under `account_lifetime`. Dated numbers in the Update Log are historical checkpoints, not current totals; do not copy them into homepage code, agent summaries, or a new baseline. This keeps future agents from treating an older evidence entry—such as the initial July 9 parser checkpoint—as the live ledger.
...
- `account_lifetime` is the canonical whole-account counter. Refresh it with `python bin/refresh_codex_account.py --write`, preserve the authenticated response timestamp, and mark the source-local current calendar day partial.
...
- The homepage's playful cost line uses `api_cost_equivalence`. The old CodexBar screenshot ratio remains historical diagnostic provenance only and is never rendered or used to price current public stats.
...
- Authenticated Codex account activity at `GET /wham/profiles/me`, used for the canonical account-lifetime token, conversation, and recent-daily snapshot.
...
- Model-policy v3 adds exact acknowledgments for one interrupted, non-site `gpt-5.6-sol/low` personal-metrics startup turn and three provider-managed `codex-auto-review/low` turns for VariationWeaver-Canvas or semantic-scaffolding-map. All original model/effort values and per-turn provenance remain visible; the fail-closed inventory reports 11 acknowledged and zero unacknowledged deviations.
- Pending public totals: 348 revamp commits, 5.69B rounded retained-session tokens, 408 rounded agent-hours, 3414 kWh, about 2.1 ten-year urban trees' stored-carbon equivalent, and about $4.9K under the request-aware API-rate replay. The desk window remains 119 path-scoped commits; its time-window estimate moved to 4.6B rounded tokens and 294 rounded hours.
- Evidence: `python bin/audit_agentic_usage.py --write --include-pending-commit` scanned 633 local and 251 repo-attributed leaf sessions, retained 39,942 additive usage-event sources after skipping 218,645 copied/repeated snapshots and 88,113 fork-preamble events, and observed 761 retained post-cutover turns. The full Python suite passed 110 tests, including 34 focused ledger-audit tests, and the exact browser reproduction produced `windowJump` with no runtime errors after forcing the canvas above the viewport.
