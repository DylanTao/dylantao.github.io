...
- Model-policy v5 adds exact acknowledgments for one interrupted, non-site `gpt-5.6-sol/low` personal-metrics startup turn and seven provider-managed `codex-auto-review/low` turns for VariationWeaver-Canvas or semantic-scaffolding-map. All original model/effort values and per-turn provenance remain visible; the fail-closed inventory reports 15 acknowledged and zero unacknowledged deviations.
- Pending public totals: 349 revamp commits, 5.69B rounded retained-session tokens, 408 rounded agent-hours, 3414 kWh, about 2.1 ten-year urban trees' stored-carbon equivalent, and about $4.9K under the request-aware API-rate replay. The desk window remains 119 path-scoped commits; its time-window estimate moved to 4.61B rounded tokens and 294 rounded hours.
- Evidence: `python bin/audit_agentic_usage.py --write --include-pending-commit` scanned 634 local and 252 repo-attributed leaf sessions, retained 39,994 additive usage-event sources after skipping 218,646 copied/repeated snapshots and 89,269 fork-preamble events, and observed 766 retained post-cutover turns. The full Python suite passed 111 tests, including 35 focused ledger-audit tests, and the exact browser reproduction produced `windowJump` with no runtime errors after forcing the canvas above the viewport.
