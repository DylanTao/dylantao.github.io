...
The first Linux release run exposed timing assumptions in the new camera and existing pause checks. Camera stress now sends short repeat-event bursts through the browser handler, with a real focused key press and a completed frame between bursts; all orbit, envelope and screenshot assertions remain. Pause verification waits for the final requested frame to drain before asserting that the frame count stays unchanged. Offscreen verification also waits until the canvas has left the viewport rather than sampling during page scrolling. The scene evidence reports pending-frame state for this synchronization; rendering behavior and public controls are unchanged. The software-WebGL camera and desktop pause checks passed, followed by a passing laptop offscreen/recovery rerun after its scrolling correction. The clean production rebuild, 163 Python checks and 23 JavaScript checks also passed before the release matrix is repeated.
