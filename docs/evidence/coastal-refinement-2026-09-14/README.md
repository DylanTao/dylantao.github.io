...
- Software rendering also exposed frame-dependent camera settling and slow room travel. Camera easing now uses elapsed time, pause settles to a composed still, and room animation accepts bounded intervals down to 4 fps. The Linux production checks passed pause/offscreen/hidden-tab recovery and the complete walk up the stairs into the onsen, including the final seated contact. The follow-up production build passed; existing browser assertions and timing deadlines were retained.
- Linux software rendering exposed the same frame cap in P's gesture playback: a curious head tilt barely advanced at low frame rates. Gesture time now follows frame intervals down to 4 fps, while antenna springs use bounded substeps and suspension gaps stay capped. The Linux Chromium companion suite passed **20 cases with two viewport-specific skips** across desktop and mobile, including gesture timing, context recovery, old nap recovery, room/page ownership, and the 10 fps journey deadline. Node checks also cover expressive poses at 5, 10, and 60 fps. The follow-up production build passed in 62.96 seconds.
...
- Linux CI runs each of the four public viewports and its site/scene streams on isolated runners, with one browser worker each. The Chromium/WebKit legacy scope runs once on the desktop site runner. This preserves all checks and assets while allowing the full route and graphics coverage to finish concurrently; artifact names retain both the viewport and stream.
- The Linux footer follow-up passed **nine cases with one viewport skip**: all four themes, full-width bottom placement, reversible reveal, lazy loading, offscreen pause, keyboard orbit, reduced motion, context recovery, missing-model fallback, and no-JavaScript content. Separate mesh/decoder retry checks also passed. Mobile-emulated pause/recovery and pinch zoom passed on the desktop GPU.
...
- Python unit suite: **163 passed**. Node routine/navigation/companion suite: **18 passed**. Style contract passed.
