...
- Cinematic layer, slice 3 (2026-09-05): the blog index becomes an editorial list (date column beside each entry, one text measure, thumbnails hanging at the right, an accent hairline and title underline on hover, and titles back at heading size after the display rule was scoped to `h1.post-title`); publications put the papers first with the Wall of Rejection after them, carry the same scroll thread beside the year headings, grow the Scholar citation bars into view, and share the header field and title drift.
- Sirui's review of slices 1 to 3 (2026-09-05): the side threads on the homepage and publications, the gradient field behind page titles, the title drift, and the papers-first reorder read as confusion rather than value, so they are gone and the Wall of Rejection leads the publications page again; the footer link row now shares the sentence's face with hairline dividers. In their place, the Research Focus sketch draws itself in on first sight, changes modes through an iris that opens from the pointer, carries a soft glow on wide screens, and lifts briefly when the reader scrolls past; the Paper Constellation opens with staggered nodes and solid edges that draw themselves from their source, papers breathe on a slow cycle, and the active paper and its edges glow. Context-Aware Encoding and GraphHSCN opt into the layer for the hero entrance, reading-block reveals, and the card-to-hero morph (What Happened and Why keeps its own trace layout, which the layer has nothing to add to), and three blog descriptions were rewritten in plain prose.
...
- Homepage scroll story and the remaining todos (2026-09-05, after Sirui asked for a less vanilla homepage scroll and a simpler heuristics file): on wide screens with the live layer, the Research Focus sketch pins beside its three lens notes, all three stay visible in reading order, and the note in the middle of the viewport picks the lens through the same buttons a click uses (`data-research-scroll-story`, set by `cinematic.js`, keeps `research-motion.js` from hiding notes); the thesis paragraph arrives word by word as the reader scrolls into it and never drops below a readable tint; the recruiting steps rise in sequence; and shallow parallax separates layers that belong together (the principle note behind the thesis, the why-now framing behind its claims, a case page's figure well beside its copy, blog figures among their words), whole and uncropped, under a third of the block's height. Blog posts load the cinematic layer through a `_config.yml` default, so the title block arrives first and each block of a note rises into view once. The projects index shows Research and Fun as real headings with the two Fun groups a step smaller. `WEBSITE_DESIGN_HEURISTICS.md` went from 443 to 328 lines with every canonical heading and pinned phrase intact: Stripe lessons folded into the inspiration boundaries, the playful, materiality, style-opportunity, and annotation sections folded into one Objects And Materiality section, project images into Visual Hierarchy, brand-mark rules pointed at `docs/material-lite-revamp.md`, and the twelve desk-scene intent records moved to `docs/homepage-desk-scene-brief.md` under Accepted Intent Record.
