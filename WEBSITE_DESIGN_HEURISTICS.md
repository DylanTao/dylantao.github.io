...
- Build-rhythm evidence: commits describe cadence, while additions and deletions describe change magnitude and direction; neither substitutes for the other. Code history appears only after an exact schema-4 source proves complete UTC coverage with one row per date from the pinned lifetime start through the latest completed day. Until then, show one compact `Code history is being rebuilt.` state and no inherited totals; an invalid refresh preserves only a previously valid snapshot. Publish two commit counts, not one: `commits` reproduces the GitHub contribution graph (default branch plus `gh-pages`, merges included) so the page reconciles with the profile, and `authored_commits` is the non-merge, non-deploy subset that alone carries added and removed lines, because a merge diff restates its branch and a deploy restates the whole generated site. Name every source (`Personal`, later `Intern work`), key points by source id, stack them with a legend that doubles as the filter, and omit a source from days outside its own coverage rather than padding it with zeroes. Keep the personal code views synchronized to the same selected day, provide readable and literal scales plus an exact-date table, and label them as build rhythm rather than productivity or causal evidence. On compact screens, shorten chart headings without dropping units or transform names, keep punctuation grouped with the value it introduces so wrapping never leaves an orphaned separator, and name the horizontal reading path when the exact table preserves more columns than the viewport can show. A sanitized personal agent completed-day series may share the date axis only when its own coverage and source-count provenance tuple validate, remains separate from the repo-scoped retained-session estimate, and leaves unobserved history blank instead of inventing zeros. When fixed Codex and Claude families are available, their priors and daily values must conserve the combined series; show them as stacked cumulative areas plus exact labels, keep both Codex accounts combined, and limit Claude to retained usage observed on this laptop. Readable/Literal may change number formatting, not an evidence boundary. Account identities and per-account histories remain private; rounded daily repo points describe a separate retained build rhythm rather than exact usage, quality, or productivity.
