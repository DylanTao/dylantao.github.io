...
- Project-preview feedback: FLIP remains the projects index's only layout motion because it preserves card identity while one preview expands. Opening and closing also update a polite status, keyboard entry moves to the primary action, and any focused control hidden by dismissal returns focus to the card trigger; reduced motion changes the layout immediately.
