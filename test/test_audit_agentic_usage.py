...
# Acknowledgment keys are session turn IDs, which are UUIDv7 values.
TURN_ID_PATTERN = r"^[0-9a-f]{8}-[0-9a-f]{4}-7[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$"
...
class SessionAccountingTests(unittest.TestCase):
    ...
    def test_acknowledgment_policy_has_complete_versioned_turn_entries(self) -> None:
        # Structural invariants only. An exact entry count and a pinned policy version
        # invalidate themselves on every acknowledgment append, which turns a routine
        # ledger refresh into a red build until someone hand-bumps the literal. The
        # checks below catch a malformed or duplicated append without that treadmill.
        self.assertIsInstance(audit.MODEL_DEVIATION_ACKNOWLEDGMENT_POLICY_VERSION, int)
        ...
        for turn_id, policy in audit.MODEL_DEVIATION_ACKNOWLEDGMENTS.items():
            with self.subTest(turn_id=turn_id):
                self.assertRegex(turn_id, TURN_ID_PATTERN)
                ...
                signature = (policy["timestamp"], policy["model"], policy["effort"])
                self.assertNotIn(
                    signature,
                    signatures,
                    f"{turn_id} repeats the acknowledgment signature already claimed by {signatures.get(signature)}",
                )
                signatures[signature] = turn_id
    ...
    def test_policy_v42_external_research_turns_are_acknowledged_by_exact_signature(self) -> None:
        turns = audit.MODEL_DEVIATION_ACKNOWLEDGMENT_V42_EXTERNAL_RESEARCH_TURNS
        self.assertEqual(
        ...
        for ordinal, row in enumerate(turns, start=1):
            turn_id, exact_timestamp, *_ = row
            policy = audit.MODEL_DEVIATION_ACKNOWLEDGMENTS[turn_id]
            timestamp = audit.parse_timestamp(exact_timestamp)
            assert timestamp is not None
            contexts[turn_id] = audit.TurnContextRecord(
                timestamp=timestamp,
                leaf_session_id=f"policy-v42-{ordinal}",
                turn_id=turn_id,
                model=policy["model"],
                effort=policy["effort"],
        ...
        for row in turns:
            turn_id, _timestamp, leaf_session, spawn_task, runtime_cwd, response_at = row
            policy = audit.MODEL_DEVIATION_ACKNOWLEDGMENTS[turn_id]
            self.assertEqual(policy["model"], "gpt-5.6-sol")
            self.assertEqual(policy["effort"], "max")
            self.assertIn(f"leaf session {leaf_session}", policy["provenance"])
            ...
            self.assertIn("substantive research/acquisition subagent", policy["reason"])
            self.assertIn("AGENTS model policy requires it", policy["reason"])
            self.assertIn(
                "did not perform site development or change the site's declared default",
                policy["reason"],
    ...
    def test_policy_v43_external_research_followups_are_acknowledged(self) -> None:
        turns = audit.MODEL_DEVIATION_ACKNOWLEDGMENT_V43_EXTERNAL_RESEARCH_FOLLOWUPS
        self.assertEqual(
            tuple(row[0] for row in turns),
            (
        ...
        for ordinal, row in enumerate(turns, start=1):
            ...
            contexts[turn_id] = audit.TurnContextRecord(
                timestamp=timestamp,
                leaf_session_id=f"policy-v43-{ordinal}",
                turn_id=turn_id,
                model=policy["model"],
                effort=policy["effort"],
                path=Path(f"policy-v43-{ordinal}.jsonl"),
                ordinal=ordinal,
            )
        ...
        for row in turns:
            ...
            self.assertIn("read-only handoff turn", policy["reason"])
            self.assertIn("AGENTS model policy", policy["reason"])
            self.assertIn("did not perform site development", policy["reason"])

    def test_policy_v44_cleanup_audit_turns_are_acknowledged(self) -> None:
        turns = audit.MODEL_DEVIATION_ACKNOWLEDGMENT_V44_CLEANUP_AUDIT_TURNS
        self.assertEqual(
            tuple(row[0] for row in turns),
            (
        ...
        for ordinal, row in enumerate(turns, start=1):
            ...
            contexts[turn_id] = audit.TurnContextRecord(
                timestamp=timestamp,
                leaf_session_id=f"policy-v44-{ordinal}",
                turn_id=turn_id,
                model=policy["model"],
                effort=policy["effort"],
                path=Path(f"policy-v44-{ordinal}.jsonl"),
                ordinal=ordinal,
            )
        ...
        self.assertEqual(tracking["post_cutover_unacknowledged_deviation_count"], 0)
        self.assertEqual(tracking["post_cutover_observed_breakdown"], {"gpt-5.6-terra/high": 5})
        self.assertEqual(audit.model_tracking_check_messages(tracking), [])

        for row in turns:
            turn_id, _timestamp, leaf_session, agent_path, runtime_cwd, response_at = row
            policy = audit.MODEL_DEVIATION_ACKNOWLEDGMENTS[turn_id]
            self.assertEqual(policy["model"], "gpt-5.6-terra")
            self.assertEqual(policy["effort"], "high")
            self.assertIn(f"leaf session {leaf_session}", policy["provenance"])
            ...
            self.assertIn("cleanup and provenance-audit agent", policy["reason"])
            self.assertIn("did not perform site development", policy["reason"])
            self.assertIn("declared default", policy["reason"])

    def test_policy_v45_lifetime_readout_turns_are_acknowledged(self) -> None:
        turns = audit.MODEL_DEVIATION_ACKNOWLEDGMENT_V45_LIFETIME_READOUT_TURNS
        self.assertEqual(
            tuple(row[0] for row in turns),
            (
        ...
        for ordinal, row in enumerate(turns, start=1):
            ...
            contexts[turn_id] = audit.TurnContextRecord(
                timestamp=timestamp,
                leaf_session_id=f"policy-v45-{ordinal}",
                turn_id=turn_id,
                model=policy["model"],
                effort=policy["effort"],
                path=Path(f"policy-v45-{ordinal}.jsonl"),
                ordinal=ordinal,
            )
        ...
        self.assertEqual(tracking["post_cutover_acknowledged_deviation_count"], 2)
        self.assertEqual(tracking["post_cutover_unacknowledged_deviation_count"], 0)
        self.assertEqual(tracking["post_cutover_observed_breakdown"], {"gpt-5.6-sol/high": 2})
        self.assertEqual(audit.model_tracking_check_messages(tracking), [])
        ...
        for row in turns:
            ...
            self.assertIn("explicitly delegated", policy["reason"])
            self.assertIn("bounded override", policy["reason"])
            self.assertIn("declared ultra default", policy["reason"])

    def test_policy_v46_research_workflow_turns_are_acknowledged(self) -> None:
        turns = audit.MODEL_DEVIATION_ACKNOWLEDGMENT_V46_RESEARCH_WORKFLOW_TURNS
        self.assertEqual(
            tuple(row[0] for row in turns),
            (
        ...
        contexts = {}
        for ordinal, row in enumerate(turns, start=1):
            turn_id, exact_timestamp, model, effort, *_ = row
            timestamp = audit.parse_timestamp(exact_timestamp)
            assert timestamp is not None
            contexts[turn_id] = audit.TurnContextRecord(
                timestamp=timestamp,
                leaf_session_id=f"policy-v46-{ordinal}",
                turn_id=turn_id,
                model=model,
                effort=effort,
                path=Path(f"policy-v46-{ordinal}.jsonl"),
                ordinal=ordinal,
            )
        ...
        self.assertEqual(
            ...
            {"gpt-5.6-sol/high": 6, "gpt-5.6-sol/max": 1},
        )
        self.assertEqual(audit.model_tracking_check_messages(tracking), [])
        ...
        for row in turns:
            ...
            self.assertEqual(policy["effort"], effort)
            self.assertIn(f"leaf session {leaf_session}", policy["provenance"])
            self.assertIn(f"agent path {agent_path}", policy["provenance"])
            self.assertIn(f"exact runtime cwd {runtime_cwd}", policy["provenance"])
            self.assertIn(
            ...
            self.assertIn("separate semantic-scaffolding-map workflow", policy["reason"])
            self.assertIn("did not perform site development", policy["reason"])
            self.assertIn("declared default", policy["reason"])

    def test_policy_v47_pdf_queue_review_turn_is_acknowledged(self) -> None:
        (
            turn_id,
            exact_timestamp,
            leaf_session,
            ...
            runtime_cwd,
            response_at,
        ) = audit.MODEL_DEVIATION_ACKNOWLEDGMENT_V47_PDF_QUEUE_REVIEW_TURN
        policy = audit.MODEL_DEVIATION_ACKNOWLEDGMENTS[turn_id]
        timestamp = audit.parse_timestamp(exact_timestamp)
        assert timestamp is not None
        ...
        tracking = audit.build_model_tracking(
            audit.UsageDataset(
                ...
                usage_events=[],
                contexts_by_turn={
                    turn_id: audit.TurnContextRecord(
                        timestamp=timestamp,
                        leaf_session_id=leaf_session,
                        turn_id=turn_id,
                        model=policy["model"],
                        effort=policy["effort"],
                        path=Path("policy-v47.jsonl"),
                        ordinal=1,
                    )
                },
                source_counts={},
        ...
        self.assertEqual(tracking["post_cutover_acknowledged_deviation_count"], 1)
        self.assertEqual(tracking["post_cutover_unacknowledged_deviation_count"], 0)
        self.assertEqual(tracking["post_cutover_observed_breakdown"], {"gpt-5.6-sol/high": 1})
        self.assertEqual(audit.model_tracking_check_messages(tracking), [])
        self.assertIn(f"leaf session {leaf_session}", policy["provenance"])
        self.assertIn(f"agent path {agent_path}", policy["provenance"])
        self.assertIn(f"exact runtime cwd {runtime_cwd}", policy["provenance"])
        self.assertIn(f"first scoped assistant response at {response_at}", policy["provenance"])
        self.assertIn("PDF-queue patch review", policy["reason"])
        self.assertIn("did not perform site development", policy["reason"])

    def test_policy_v48_research_workflow_turns_are_acknowledged(self) -> None:
        turns = audit.MODEL_DEVIATION_ACKNOWLEDGMENT_V48_RESEARCH_WORKFLOW_TURNS
        self.assertEqual(
            tuple(row[0] for row in turns),
            (
        ...
        for ordinal, row in enumerate(turns, start=1):
            ...
            contexts[turn_id] = audit.TurnContextRecord(
                timestamp=timestamp,
                leaf_session_id=f"policy-v48-{ordinal}",
                turn_id=turn_id,
                model=model,
                effort=effort,
                path=Path(f"policy-v48-{ordinal}.jsonl"),
                ordinal=ordinal,
            )
        ...
        for row in turns:
            ...
            self.assertIn("separate semantic-scaffolding-map", policy["reason"])
            self.assertIn("did not perform site development", policy["reason"])
            self.assertIn("declared default", policy["reason"])
            if category == "canary":
                self.assertIn("exact no-tools runtime-attestation canary", policy["reason"])
            else:
                self.assertIn("Bounded delegated", policy["reason"])
                self.assertIn("coordinator/provider-selected effort", policy["reason"])

    def test_policy_v49_research_cache_turn_is_acknowledged(self) -> None:
        (
            turn_id,
            exact_timestamp,
            leaf_session,
            ...
            runtime_cwd,
            response_at,
        ) = audit.MODEL_DEVIATION_ACKNOWLEDGMENT_V49_RESEARCH_CACHE_TURN
        policy = audit.MODEL_DEVIATION_ACKNOWLEDGMENTS[turn_id]
        timestamp = audit.parse_timestamp(exact_timestamp)
        assert timestamp is not None
        ...
        self.assertIn(f"exact runtime cwd {runtime_cwd}", policy["provenance"])
        self.assertIn(f"first scoped assistant response at {response_at}", policy["provenance"])
        self.assertIn("explicitly delegated engineering subagent", policy["reason"])
        self.assertIn("transaction-scoped read-only ledger cache", policy["reason"])
        self.assertIn("did not perform site development", policy["reason"])

    def test_policy_v50_research_followup_turns_are_acknowledged(self) -> None:
        turns = audit.MODEL_DEVIATION_ACKNOWLEDGMENT_V50_RESEARCH_FOLLOWUP_TURNS
        self.assertEqual(
            tuple(row[0] for row in turns),
            (
        ...
        for ordinal, row in enumerate(turns, start=1):
            ...
            contexts[turn_id] = audit.TurnContextRecord(
                ...
                model="gpt-5.6-terra",
                effort="high",
                path=Path(f"policy-v50-{ordinal}.jsonl"),
                ordinal=ordinal,
            )
    ...
    def test_policy_v51_turns_are_acknowledged_by_exact_signature(self) -> None:
        turns = audit.MODEL_DEVIATION_ACKNOWLEDGMENT_V51_TURNS
        self.assertEqual(
            tuple(row[0] for row in turns),
            (
        ...
        for ordinal, row in enumerate(turns, start=1):
            ...
            contexts[turn_id] = audit.TurnContextRecord(
                ...
                model=model,
                effort=effort,
                path=Path(f"policy-v51-{ordinal}.jsonl"),
                ordinal=ordinal,
            )
    ...
    def test_policy_v52_research_followups_are_acknowledged(self) -> None:
        turns = audit.MODEL_DEVIATION_ACKNOWLEDGMENT_V52_RESEARCH_FOLLOWUP_TURNS
        self.assertEqual(
            tuple(row[0] for row in turns),
            (
        ...
        for ordinal, row in enumerate(turns, start=1):
            ...
            contexts[turn_id] = audit.TurnContextRecord(
                ...
                model=model,
                effort=effort,
                path=Path(f"policy-v52-{ordinal}.jsonl"),
                ordinal=ordinal,
            )
    ...
    def test_policy_v54_site_followups_are_acknowledged(self) -> None:
        turns = audit.MODEL_DEVIATION_ACKNOWLEDGMENT_V54_SITE_FOLLOWUP_TURNS
        self.assertEqual(
            tuple(row[0] for row in turns),
            (
        ...
        for ordinal, row in enumerate(turns, start=1):
            ...
            contexts[turn_id] = audit.TurnContextRecord(
                ...
                model="gpt-5.6-sol",
                effort="high",
                path=Path(f"policy-v54-{ordinal}.jsonl"),
                ordinal=ordinal,
            )
    ...
    def test_policy_v55_token_ui_release_turns_are_acknowledged(self) -> None:
        turns = audit.MODEL_DEVIATION_ACKNOWLEDGMENT_V55_TOKEN_UI_RELEASE_TURNS
        self.assertEqual(
            tuple(row[0] for row in turns),
            (
        ...
        for ordinal, row in enumerate(turns, start=1):
            ...
            contexts[turn_id] = audit.TurnContextRecord(
                ...
                model="gpt-5.6-sol",
                effort="high",
                path=Path(f"policy-v55-{ordinal}.jsonl"),
                ordinal=ordinal,
            )
    ...
    def test_model_tracking_does_not_use_unrelated_repo_contexts(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            site_tracking = audit.build_model_tracking(audit.prepare_usage_dataset(repo_sessions))

        self.assertEqual(repo_sessions, {})
        self.assertEqual(site_tracking["scope"], audit.MODEL_TRACKING_SCOPE)
        self.assertEqual(site_tracking["status"], "unobserved")
        self.assertEqual(site_tracking["post_cutover_turns_observed"], 0)
        self.assertEqual(site_tracking["post_cutover_deviation_count"], 0)
        self.assertEqual(len(audit.model_tracking_check_messages(site_tracking)), 1)
...
class TokenRhythmTests(unittest.TestCase):
    ...
    def test_freshness_ignores_matching_open_day_token_rhythm_counter_drift(self) -> None:
        open_day = date(2026, 8, 1)
        rhythm = {
            "schema": 1,
            "label": "Retained-session estimate",
            "units": "estimated tokens",
            "grain": "day",
            "aggregation": "cumulative",
            "method": "deduplicated_retained_logs",
            "since": "2026-07-31",
            "updated_at": open_day.isoformat(),
            "confidence": "estimate",
            "privacy_note": "Rounded daily cumulative estimates only.",
            "points": [
                {"date": "2026-07-31", "token_count": 100_000_000, "tokens_label": "100M"},
                {"date": open_day.isoformat(), "token_count": 110_000_000, "tokens_label": "110M"},
            ],
        }

        for scope_name in ("total", "local_lifetime"):
            with self.subTest(scope_name=scope_name):
                current = {scope_name: {"token_rhythm": rhythm}}
                proposed = copy.deepcopy(current)
                proposed[scope_name]["token_rhythm"]["points"][-1].update(
                    {"token_count": 120_000_000, "tokens_label": "120M"}
                )

                self.assertEqual(
                    audit.check_public_freshness(current, proposed, open_day=open_day),
                    [],
                )
...
class PendingChangesTests(unittest.TestCase):
    ...
    def test_named_model_tracking_scope_migration_accepts_narrower_site_evidence(self) -> None:
        previous_tracking = {
            "scope": "all_local_retained_contexts",
            "status": "acknowledged_deviations",
            "post_cutover_turns_observed": 3_460,
            "post_cutover_acknowledged_deviation_count": 522,
            "post_cutover_deviations": [{"turn_id": "external-turn"}],
        }
        site_tracking = {
            "scope": audit.MODEL_TRACKING_SCOPE,
            "status": "aligned",
            "post_cutover_turns_observed": 2_000,
            "post_cutover_acknowledged_deviation_count": 0,
            "post_cutover_deviations": [],
        }

        merged = audit.merge_model_tracking_data(previous_tracking, site_tracking)

        self.assertEqual(merged, site_tracking)
        self.assertIsNot(merged, site_tracking)
